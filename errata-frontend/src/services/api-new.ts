...
export const apiService = {
  ...
  getAllErrors: async (page: number = 1, limit: number = 20, api?: string, severity?: string): Promise<SearchResponse> => {
    return apiService.searchErrors('', page, limit, api);
  },
  ...
  getAPIStats: async (): Promise<APIStats[]> => {
    ...
    for (const api of apis) {
      try {
        ...
        const errors = response.data.errors || [];
        const categories = [...new Set(errors.map((error: any) => getCategory(error.code)))] as string[];
...
function getCategory(code: string): string {
  if (code.includes('authentication') || code.includes('api_key') || code.includes('bad_credentials')) return 'Authentication';
  if (code.includes('card') || code.includes('payment') || code.includes('declined')) return 'Payment';
  if (code.includes('parameter') || code.includes('invalid') || code.includes('validation')) return 'Validation';
  if (code.includes('rate_limit') || code.includes('rate_limit_exceeded')) return 'Rate Limiting';
  if (code.includes('account') || code.includes('balance')) return 'Account';
  if (code.includes('webhook') || code.includes('signature')) return 'Security';
  if (code.includes('not_found') || code.includes('repository')) return 'Resources';
  return 'General';
}
...
function getTags(code: string): string[] {
  ...
  if (code.includes('github')) tags.push('github');
  if (code.includes('stripe')) tags.push('stripe');
  return tags;
}
